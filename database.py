...
class connect_data():
    ...
    def read_data(self):
        ...
        result = []
        for data in datas:
	    result.append({r"cards_name":data.cards_name,r"num_cards":" "+data.num_cards,r"host_name":data.host_name,r"driver":data.driver,r"osenv":'Ubuntu'+data.osenv+'.04',r"ip":data.ip,"alive":data.alive,r"nowtime":str(data.nowtime)[5:-7],"mac":data.mac,"user":data.user,"desc":data.desc,"bmc":data.bmc,"bmclo":data.bmclo})
